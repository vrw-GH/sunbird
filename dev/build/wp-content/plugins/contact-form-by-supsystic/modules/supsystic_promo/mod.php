...
class supsystic_promoCfs extends moduleCfs {
	...
	public function addEditTab( $tabs ) {
		if(!$this->isPro()) {
			$tabs['cfsFormConditionalLogic'] = array(
				...
				<br>
				<a style="margin-top:15px;" href="'. $this->generateMainLink('utm_source=plugin&utm_medium=conditional_logic&utm_campaign=forms'). '" target="_blank"><img style="max-width:800px; width: 100%; height: auto;" src="'. CFS_ASSETS_PATH. 'forms/promo/img/logic.png" /></a>',
				'fa_icon' => 'fa-flask',
				'sort_order' => 90,
			);
		}
		...
		if(!frameCfs::_()->getModule('ab_testing')) {
			$tabs['cfsFormAbTesting'] = array(
				...
				'content' => '<a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=abtesting&utm_campaign=forms'). '" target="_blank" class="button button-primary">'
					...
					. '<img style="max-width: 100%;" src="'. CFS_ASSETS_PATH. 'forms/promo/img/AB-testing-pro.jpg" />'
				. '</a>',
		...
		if(!frameCfs::_()->getModule('subscribe')) {
			$tabs['cfsFormSubscribe'] = array(
				...
				'content' => '<a href="'. $this->generateMainLink('utm_source=plugin&utm_medium=subscribe&utm_campaign=forms'). '" target="_blank" class="button button-primary">'
					...
					. '<img style="max-width: 100%;" src="'. CFS_ASSETS_PATH. 'forms/promo/img/subscribe.png" />'
				. '</a>',
				'fa_icon' => 'fa-users',
				'sort_order' => 50,
			);
	...
	public function getLoveLink($show = 'hide') {
		...
		if ($show == 'show') {
			return '<a title="' . $title . '" style="border:none; color: #26bfc1 !important; font-size: 9px; display: block; float: right; padding-right: 10px;" href="https://supsystic.com/plugins/contact-form-plugin/?utm_medium=love_link_show" target="_blank">' . $title . '</a>' . '<div style="clear: both;"></div>';
		} else if ($show == 'hide') {
