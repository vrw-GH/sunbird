...
const templates = {
  default: {
    pageviews: ( curr, prev ) => ({
      ...
      subtitle: __( '%s pageviews per session', 'burst-statistics' ).replace(
        '%s',
        formatNumber( curr.pageviews / curr.sessions )
      ),
    ...
    sessions: ( curr, prev ) => ({
      ...
      subtitle: __( '%s per session', 'burst-statistics' ).replace(
        '%s',
        formatTime( ( curr.pageviews / curr.sessions ) * curr.avg_time_on_page )
      ),
  ...
  goalSelected: {
    conversions: ( curr, prev ) => ({
      ...
      subtitle: __( '%s of pageviews converted', 'burst-statistics' ).replace(
        '%s',
        getPercentage( curr.conversion_rate, 100 )
      ),
    ...
    pageviews: ( curr, prev ) => ({
      title: __( 'Pageviews', 'burst-statistics' ),
      subtitle: __( '%s pageviews per conversion', 'burst-statistics' ).replace(
        '%s',
        formatNumber( curr.pageviews / curr.conversions )
      ),
      value: formatNumber( curr.pageviews ),
      exactValue: curr.pageviews
    }),
    sessions: ( curr, prev ) => ({
      title: __( 'Sessions', 'burst-statistics' ),
      subtitle: __( '%s sessions per conversion', 'burst-statistics' ).replace(
        '%s',
        formatNumber( curr.sessions / curr.conversions )
      ),
      value: formatNumber( curr.sessions ),
      exactValue: curr.sessions
    }),
    visitors: ( curr, prev ) => ({
      title: __( 'Visitors', 'burst-statistics' ),
      subtitle: __( '%s visitors per conversion', 'burst-statistics' ).replace(
        '%s',
        formatNumber( curr.visitors / curr.conversions )
      ),
      value: formatNumber( curr.visitors ),
      exactValue: curr.visitors
    })
