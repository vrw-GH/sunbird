...
class Login_Lockout extends Component {
	...
	public function show_attempt_left( $user, $username ) {
		...
		if ( ! is_wp_error( $user ) && $user instanceof WP_User
			...
			$msg    = sprintf(
				'<strong>%s:</strong> %s',
				esc_html__( 'Error', 'defender-security' ),
			...
			if ( $attempt < $this->model->attempt ) {
				$user->add(
					...
					/* translators: %d: Count of attempts. */
						esc_html__( '%d login attempts remaining', 'defender-security' ),
	...
	public function process_fail_attempt_compatibility( $username ) {
		...
		if ( in_array( $username, $this->model->get_blacklisted_username(), true ) ) {
			...
			$errors = new WP_Error( 'dummy_failed', esc_html__( 'Dummy data.', 'defender-security' ) );
		}
	...
	public function log_event( $ip, $username, $scenario, $message = '' ) {
		...
		switch ( $scenario ) {
			...
			default:
				...
				$model->log  = ( '' !== $message )
					? $message
					: esc_html__( 'Lockout occurred: Too many failed login attempts', 'defender-security' );
				break;
		}
		$model->save();
