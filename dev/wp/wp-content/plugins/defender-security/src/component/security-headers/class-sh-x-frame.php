...
class Sh_X_Frame extends Security_Header {
	...
	public function get_misc_data() {
		...
		return array(
			'intro_text' => sprintf(
				...
				esc_html__( 'The X-Frame-Options HTTP response header controls whether or not a browser can render a webpage inside a %1$s or %2$s tag. Websites can avoid clickjacking attacks by ensuring that their content isn`t embedded into other websites.', 'defender-security' ),
				'<frame>, <iframe>',
				'<object>'
			),
			/**
	...
	public function get_title() {
		return esc_html__( 'X-Frame-Options', 'defender-security' );
	}
}
