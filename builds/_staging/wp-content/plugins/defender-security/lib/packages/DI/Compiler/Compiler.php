...
use function chmod;
use WPMU_DEV\Defender\Vendor\DI\Definition\ArrayDefinition;
use WPMU_DEV\Defender\Vendor\DI\Definition\DecoratorDefinition;
...
use WPMU_DEV\Defender\Vendor\DI\Definition\FactoryDefinition;
use WPMU_DEV\Defender\Vendor\DI\Definition\ObjectDefinition;
use WPMU_DEV\Defender\Vendor\DI\Definition\Reference;
use WPMU_DEV\Defender\Vendor\DI\Definition\Source\DefinitionSource;
...
use WPMU_DEV\Defender\Vendor\Laravel\SerializableClosure\Support\ReflectionClosure;
use function rename;
...
class Compiler
{
     ...
     * if PHP-WPMU_DEV\Defender\Vendor\DI configuration does not change.
     *
     * @var int
     */
    ...
    private function compileDefinition(string $entryName, Definition $definition) : string
    {
        ...
        switch (true) {
            ...
            case $definition instanceof StringDefinition:
                ...
                $code = 'return \WPMU_DEV\Defender\Vendor\DI\Definition\StringDefinition::resolveExpression(' . $entryName . ', ' . $expression . ', $this->delegateContainer);';
                break;
        ...
        if (!$isOptional) {
            throw new \WPMU_DEV\Defender\Vendor\DI\Definition\Exception\InvalidDefinition("The environment variable '{$definition->getVariableName()}' has not been defined");
        }
        return $defaultValue;
PHP;
                break;
    ...
    /**
     * @throws \WPMU_DEV\Defender\Vendor\DI\Definition\Exception\InvalidDefinition
     */
